...
| Tests cover validation, isolation, conflict handling, and revisions | Passing automated suite whose assertions cover each behavior | At commit `d801999`, `npm run check` passed 96 tests across 12 files plus type, migration, infrastructure, repository-safety, history-safety, MCP-audit, and production-build checks | Verified locally | Re-run immediately before submission |
| Public repository is reproducible and contains no credentials | Clean public branch, setup/runbooks, tracked-file and full-history secret scans, ignored local secret files | GitHub reports the repository as public with `main`, the MIT license, and the public demo homepage; anonymous requests to the repository, README, and license returned HTTP 200; at commit `d801999`, repository safety passed across 88 tracked files and history safety passed across 334 reachable blobs | Verified public | Re-run both safety audits and record the final commit hash immediately before submission |
| Public video demonstrates the memory flow in under three minutes | Public video URL and manual review against the storyboard and redaction checklist | The Managed MCP recording gate is satisfied; a 2 minute 40 second storyboard, timed English captions, sanitized MCP visual, and three OBS window-capture scenes are prepared; a 7.966 second 1920 x 1080, 30 fps, audio-muted Hybrid MP4 test finalized automatically | Prepared | Confirm the test frame visually, then record, review, upload, and add the URL |
