...
| Managed MCP inspects the same memory read-only | OAuth connection scoped to one cluster, read permission only, allowlisted audit query returns the public agent run and decision chain | Read-only audit runbook and four schema-checked, data-minimized statements are prepared | Prepared | Authenticate and run the live MCP audit |
