...
Status: AWS application, direct Nova agent-memory inference, demo-session persistence, and preliminary read-only MCP audit verified live; diagnosed public Nova link fix and final revision audit pending
...
- The first paid public E2E verified CloudFront security headers, API health, isolated session creation, and CockroachDB bootstrap, then failed safely at analysis with `MODEL_OUTPUT_INVALID`. Cause-specific repair guidance was deployed, but the second paid run stopped at the same safe category. A third run with allowlisted telemetry identified `MODEL_OUTPUT_UNLINKED_CONFLICT`. The local fix deterministically links a conflict when exactly one extracted memory has a source quote copied from the conversation; ambiguous multi-memory cases remain rejected. Linked conflict candidates are normalized to `proposed` until the explicit revision or dismissal transition.
